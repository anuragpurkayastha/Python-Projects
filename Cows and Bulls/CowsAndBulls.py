...
"""
    Description:    Returns the number of cows and bulls.
    Arguments:
        -   randNum:    The randomly generated number
        -   userGuess:  The number user has guessed.
    Returns:
        -   2 item list with [numCows, numBulls]
"""
def countCowsAndBulls(randNum, userGuess):
	"""
	This function calculates the number of 'Cows' and 'Bulls' given two numbers, randNum. The function returns a list, 'CowsAndBulls', with the first element being the number of 'cows' and the second element being the number of 'bulls'.
	...
	"""
	numCows = 0     #   Counter for number of cows
	numBulls = 0    # Counter for the number of bulls
	difference = []			# List to store the digits of the each difference.

	num1_lst = numberToDigits(randNum)		# Convert both input numbers to lists containing each individual digit as one element.
	num2_lst = numberToDigits(userGuess)
